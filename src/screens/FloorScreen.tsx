...
import { ArrowRight, Grid3X3, Filter, LogOut } from 'lucide-react';
...
export const FloorScreen = () => {
    ...
    const handleQuickFree = async (e: React.MouseEvent, tableId: string) => {
        e.stopPropagation();
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                {filteredTables.map((table) => {
                    const tableTicket = kdsTickets.find(t => t.tableNumber === table.number && t.status !== 'completed');
                    return (
                        <TableCard
                            key={table.id}
                            table={table}
                            orderStatus={tableTicket?.status}
                            selected={selectedTable?.id === table.id}
                            onClick={() => handleTableClick(table)}
                            onFree={(e) => { handleQuickFree(e, table.id); }}
                        />
                    );
                })}
            </div>
